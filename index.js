...
`)).join("");p.insertAdjacentHTML("beforeend",s),g.refresh()}function b(){p.innerHTML=""}const w=document.querySelector(".form"),S=o=>{o.preventDefault();const s=o.target.elements["search-text"],t=s.value.trim();b(),y(),m(t).then(i=>{i.data.hits.length===0&&a.warning({title:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"}),s.value="",L(i.data.hits),n()}).catch(i=>{a.error({title:"Server error try later",position:"topRight"}),n()})};w.addEventListener("submit",S);
