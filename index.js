...
`)).join("");p.insertAdjacentHTML("beforeend",s),y.refresh()}function b(){p.innerHTML=""}const w=document.querySelector(".form"),S=o=>{o.preventDefault();const s=o.target.elements["search-text"],t=s.value.trim();if(t===""){l.error({title:"Please enter search text.",position:"topRight"});return}b(),g(),m(t).then(i=>{i.data.hits.length===0&&l.warning({title:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"}),s.value="",L(i.data.hits),n()}).catch(i=>{l.error({title:"Server error try later",position:"topRight"}),n()})};w.addEventListener("submit",S);
